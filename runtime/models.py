...
class RuntimeVersion(models.Model):
  ...
  judge = models.ForeignKey('Judge',
    verbose_name=_('judge on which this runtime exists'),
    on_delete=CASCADE)
...
class Judge(TimeStampedModel):
  name = models.CharField(max_length=50,
    help_text=_("Server name, hostname-style"), unique=True
  )
  ...
  online = models.BooleanField(
    verbose_name=_('judge online status'), default=False
  )
  start_time = models.DateTimeField(
    verbose_name=_('judge start time'), null=True
  )
  ping = models.FloatField(
    verbose_name=_('response time'), null=True
  )
